...
int build_jk_simd(double *vj, double *vk, double *dm, double *Et_dm, int n_dm,
                  int *shls_slice, int *ao_loc, int *jengine_loc,
                  double *Et_ij_cache, double *Et_kl_cache, int *Et_offsets,
                  double *q_cond, double *dm_cond, double cutoff,
                  int *atm, int natm, int *bas, int nbas, double *env)
{
        ...
        //         RdotE~Rt2*nprim_kl     RdotE*nprim_ij
                   Et_ij_len*Et_kl_lenp + Et_ij_lenp*dkdl +
        //         jvec_ij, rho_ij, jvec_kl, rho_kl
                   Et_ij_lenp*2 + Et_kl_lenp*2;
        __MD *buf = _mm_malloc(sizeof(__MD)*size, sizeof(__MD));
        ...
        for (_lsh0 = lsh0; _lsh0 < lsh1; _lsh0+=basblk_l) {
                ...
                for (int jsh = _jsh0; jsh < MIN(ish+1, _jsh1); jsh++) {
                        ...
                        for (int lsh = _lsh0; lsh < MIN(ksh+1, _lsh1); lsh++) {
                                ...
                                shl_quartets[count*4+0] = ish;
                                shl_quartets[count*4+1] = jsh;
                                shl_quartets[count*4+2] = ksh;
                                shl_quartets[count*4+3] = lsh;
                                count++;
                                if (count == SIMDD) {
                                        MD_jk_kernel_simd(&envs, &jk, shl_quartets, buf);
                                        count = 0;
                                }
                        } }
                } }
        ...
        for (int i = 0; i < count; i++) {
                int ish = shl_quartets[i*4+0];
                int jsh = shl_quartets[i*4+1];
                int ksh = shl_quartets[i*4+2];
                int lsh = shl_quartets[i*4+3];
                MD_jk_kernel(&envs, &jk, ish, jsh, ksh, lsh, (double *)buf);
        }
        free(buf);
        return 0;
}
